...
Future publishing must import only reviewed summaries from the existing private content publication gate, using its library version as the release. The delivery adapter is now documented in [PUBLICATION.md](PUBLICATION.md); it consumes a receipt of that existing gate. Authored-form completeness checks and private feasibility rules still belong to the private content workflow. Do not manually promote the demonstration catalogue to a real published curriculum. When quotes are implemented, freeze the catalogue release, chosen entries and formatting snapshot on the order; later school preference changes must not alter a paid order.
