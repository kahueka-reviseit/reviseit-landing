...
export async function readWorkspace(client: SupabaseClient, schoolId:string, moduleId?: string): Promise<Workspace> {
  ...
  const [catalogue, formatting, selection] = await Promise.all([
    client.from('catalogue_summaries').select('entry_id,title,topic,description,marks').eq('module_id',module.id).eq('release',module.release).order('entry_id'),
  ...
  return {...empty,
    entries:(catalogue.data || []).map(c=>({id:c.entry_id,title:c.title,topic:c.topic,description:c.description,marks:c.marks})),
